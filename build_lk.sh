...
BUILD_CROSS_COMPILE=../../android-toolchain-eabi/bin/arm-linux-androideabi-
...
FUNC_APPEND_DTB()
{
	...
	for DTS_FILE in `ls ${BUILD_KERNEL_DIR}/arch/arm/boot/dts/msm8974/${DTS_NAMES}*.dts`
	do
...
FUNC_MKBOOTIMG()
{
	...
	if ! [ -e $MKBOOTIMGTOOL ] ; then
		...
		cp $BUILD_TOP_DIR/tools/mkbootimg $MKBOOTIMGTOOL
	fi
